...
<body>
                            ...
                            <fieldset>
                                ...
                                <div class="join__Address">
                                    ...
                                    <div class="check">
                                        ...
                                        <div class="btn__style3" id="addressCheck">검색</div>
                                    </div>
                                ...
                                <button type="submit" id="submitBtn" class="btn__style3">계속</button>
                            </fieldset>
                        </form>
                    </div>
                </div>
    ...
    <script>
        ...
        function joinChecks(){
            ...
            // 생년월일 유효성 검사
            let getYouBirth = RegExp(/^(19\d{2}|20\d{2})(0[1-9]|1[0-2])(0[1-9]|[1-2][0-9]|3[0-1])$/);

            if (!getYouBirth.test($("#youBirth").val())) {
                $("#memberBirthMsg").text("생년월일이 정확하지 않습니다. (올바른 형식: YYYYMMDD, 하이픈 없이 숫자만 입력)");
                $("#youBirth").val('');
                $("#youBirth").focus();
                return false;
            }

            const year = parseInt($("#youBirth").val().substring(0, 4));
            const month = parseInt($("#youBirth").val().substring(4, 6));
            const day = parseInt($("#youBirth").val().substring(6, 8));

            if (year < 1900 || year > new Date().getFullYear() || month < 1 || month > 12 || day < 1 || day > 31) {
                $("#memberBirthMsg").text("생년월일이 유효하지 않습니다. (년도: 1900 이후, 월: 1-12, 일: 1-31)");
                $("#youBirth").val('');
                $("#youBirth").focus();
                return false;
            }
